...
try{
	...
	var min_word = document.querySelector("#min");
	var max_word = document.querySelector("#maxword");
	...
	for(var i=0; i<1000; i++){
		if(words[i].length>= min_word.value && words[i].length<= max_word.value){
			if(!eztype.checked){
				possible.push(words[i]);
			}
			else{
				var counts={};
				for(var index=0;index<words[i].length;index++){
					count = counts[words[i][index]];
	...
	while(result_list.length<10 && grand_attempts<1000){
		...
		while(count<4 && attempts<10000){
			...
			passphrase.push(possible[idx]);
			count += 1;
			length += possible[idx].length;
		}
		if(count==4){
	...
	for(var i=0;i<result_list.length;i++){
		...
		if(numsubs.checked){
			result_list[i]=result_list[i].replace(/o/g,'0');
			result_list[i]=result_list[i].replace(/e/g,'3');
			result_list[i]=result_list[i].replace(/i/g,'1');
			result_list[i]=result_list[i].replace(/l/g,'1');
		}
		...
		td.innerHTML=result_list[i];
		td2.innerHTML="Length: "+len;
		tr.appendChild(td);
		tr.appendChild(td2);
		lis.appendChild(tr);
	}
